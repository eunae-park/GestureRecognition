...
//		System.out.println(finisherIndex);
		...
		else if(finisherIndex-starterIndex < 160) //finisherIndex-starterIndex<0 || 
		{
...
//			System.out.println(numOfPressure+i + " - " + SensorValue[numOfPressure+i]); // print v1114
		...
		while(i<6)
		{
			data = split_dataBody[k].substring(sensorPos, split_dataBody[k].indexOf("\t",sensorPos+1)); // S占쎈퓠占쎈뮉 NO.
			sensorPos = sensorPos + data.length() + 1; // L or R delete 
			if(!data.equals(finger[i]))
			{
				k ++;
...
//			System.out.println("Result : " + SensorValue); // print v1114
	...
	public double[] getDatas3th(int hand) throws IOException
	{
...
//		System.out.println((numOfSensingPoints-1) + "Count : " + SensorValue[numOfSensingPoints-1]);
	...
	public double[] getDatas4th() throws IOException
	{
		
		final int packetSize = 512+256;
		//final int packetSize = 60;		
//		String finisherArr[] = {"S1", "S2", "S3", "S0"};
		String finger_L[] = {"L0", "L1", "L2", "L3", "L4", "L"};
		String starter_L = "L0";
		String finisher_L = "L0";
		String finger_R[] = {"R0", "R1", "R2", "R3", "R4", "R"};
		String starter_R = "R0";
		String finisher_R = "R0";
		
		if(serialPort == null || !serialPort.isOpen())
		{
			return null;
		}
		else if(sensingBuffer == null || sensingBuffer.length() < (packetSize + starter_L.length() + finisher_L.length()))
		{
			return null;
		}
...
//		System.out.println(sensingBuffer);
		String dataBody = null;
		int starterIndex_L = sensingBuffer.indexOf(starter_L); // 癲ル슪�릻�댆洹잙뼀占쎈Ŋ占썬굝�쐻�뜝占� S0
		int finisherIndex_L = sensingBuffer.indexOf(finisher_L, starterIndex_L+1) - 1; // single
		int starterIndex_R = sensingBuffer.indexOf(starter_R); // 癲ル슪�릻�댆洹잙뼀占쎈Ŋ占썬굝�쐻�뜝占� S0
		int finisherIndex_R = sensingBuffer.indexOf(finisher_R, starterIndex_R+1) - 1; // single

		int starterIndex=-1, finisherIndex=-1;
		
		if (starterIndex_L >=0 && starterIndex_R >=0)
		{
			if (starterIndex_L < starterIndex_R) // start index = front
				starterIndex = starterIndex_L;
			else
				starterIndex = starterIndex_R;
		}
		else
		{
			if (starterIndex_L >= 0)
				starterIndex = starterIndex_L;
			else if (starterIndex_R >= 0)
				starterIndex = starterIndex_R;
		}

		if (finisherIndex_L >=0 && finisherIndex_R >=0)
		{
			if (finisherIndex_L < finisherIndex_R) // finish index = rear
				finisherIndex = finisherIndex_R;
			else
				finisherIndex = finisherIndex_L;
		}
		else
		{
			if (finisherIndex_L >= 0) // finish index = rear
				finisherIndex = finisherIndex_L;
			else if (finisherIndex_R >= 0)
				finisherIndex = finisherIndex_R;
		}
...
//		System.out.println(finisherIndex);
		...
		int allOfSensors = numOfPressure + numOfGyro + numOfStrech;
		int i, j, k, sensorPos=0, sensorNum; // 29 is 3th
		double[] SensorValue = new double[allOfSensors*2];
		Arrays.fill(SensorValue, initialValue);
		String data;
		...
		else if(finisherIndex-starterIndex < 100) //finisherIndex-starterIndex<0 || 
		{
//			System.out.println("\n글로브 센서를 확인하세요.");
			if (starterIndex > 0)
			{
			...
			starterIndex = sensingBuffer.indexOf(starter_L);
			finisherIndex = sensingBuffer.indexOf(finisher_L, starterIndex+1) - 1;
//			System.out.println("Search2 : " + finisherIndex + " " + starterIndex+ " " + (finisherIndex - starterIndex));
			if(finisherIndex - starterIndex - 1 < 0)
			{
				return null;
...
//		else if(dataBody.indexOf(finger_L[0], 0)<0 || dataBody.indexOf(finger_L[1], 0)<0 || dataBody.indexOf(finger_L[2], 0)<0 || dataBody.indexOf(finger_L[3], 0)<0 || dataBody.indexOf(finger_L[4], 0)<0 || dataBody.indexOf(finger_L[5], 0)<0)
//		{
//			SensorValue[0] = -12345.67890;
//			return SensorValue;
//		}
		...
		String[] split_dataBody = dataBody.split("\n");	
		int split_len = split_dataBody.length;
//		System.out.println(split_len); // print v1114
		sensorNum = 0;
		
		i = 0; // left
		k = 0;
		while(i<5 & k<split_len)
		{
//			System.out.println(split_dataBody[k]);
			if(split_dataBody[k].indexOf("\t",sensorPos+1) < 0) //占쎌굙占쎌뇚筌ｌ꼶�봺
			{
