...
//		System.out.println(sensingBuffer);
		String dataBody = null;
		int starterIndex = sensingBuffer.indexOf(starter); // 筌ｃ꺂苡뀐쭪占� S0
		int finisherIndex = sensingBuffer.indexOf(finisher, starterIndex+1) - 1; // single
		if(starterIndex < 0 && finisherIndex < 0)
		{
			starterIndex = sensingBuffer.indexOf(l_starter); // 筌ｃ꺂苡뀐쭪占� S0
			...
			if(starterIndex < 0 && finisherIndex < 0)
			{
				starterIndex = sensingBuffer.indexOf(r_starter); // 筌ｃ꺂苡뀐쭪占� S0
...
//		System.out.printf("%d\t%d\t", sensingBuffer.length(), dataBody.length());
		...
		if(sensingBuffer.length() > packetSize*1000){ // �늻�쟻�맂 data set�씠 1000媛� �씠�긽�씠硫�
...
/*// 한손 연결 시 
		...
		for(i=0; i<5; i++)
		{
			...
			{
				data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1));
				SensorValue[sensorNum] = Double.parseDouble(data); // flexible - (double)Integer.parseInt(strHexValue, 16)); //16吏꾩닔瑜� 10吏꾩닔濡�
...
//			sensorNum ++;
		...
		data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1)); // S�씪 �븣�뒗 NO.
		sensorPos = sensorPos + data.length() + 1; // L or R delete 
		for(j=0; j<numOfGyro-1; j++)		//IMU sensor datas
		{
			data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1));
			SensorValue[numOfPressure+numOfStrech+j] = Double.parseDouble(data); // flexible - (double)Integer.parseInt(strHexValue, 16)); //16吏꾩닔瑜� 10吏꾩닔濡�
...
//			System.out.println(split_dataBody[k]);
			if(split_dataBody[k].indexOf("\t",sensorPos+1) < 0) //예외처리
			{
				k ++;
				sensorPos = 0;
				continue;				
...
//			System.out.println(numOfPressure+i + " - " + SensorValue[numOfPressure+i]); // print v1114
		...
		while(i<6)
		{
			data = split_dataBody[k].substring(sensorPos, split_dataBody[k].indexOf("\t",sensorPos+1)); // S에는 NO.
			sensorPos = sensorPos + data.length() + 1; // L or R delete 
			if(!data.equals(finger[i]))
			{
				k ++;
...
//		String finisherArr[] = {"S1", "S2", "S3", "S0"};
		...
		String dataBody = null;
		int starterIndex = sensingBuffer.indexOf(starter); // 筌ｃ꺂苡뀐쭪占� S0
		int finisherIndex = sensingBuffer.indexOf(starter, starterIndex+1) - 1; // 占쎈あ甕곕뜆�럮 S0
		final int startPosition = 0;
		final int initialValue = 0;
...
//		String finisherArr[] = {"S1", "S2", "S3", "S0"};
		...
		String data;
/*
		int starterIndex = sensingBuffer.indexOf(starter)-78; // S0 筌≪뼚釉섓옙苑� 域뱄옙 占쎌읈 占쎌쁽占쎌뵠嚥≪뮇苑깍옙苑� 占쎈쑓占쎌뵠占쎄숲繹먮슣占�
		int finisherIndex = sensingBuffer.indexOf(finisher, starterIndex+80) + 100; // S4 筌≪럩釉섓옙苑� 占쎈퉸占쎈뼣 占쎈릍占쎌젾占쎄쉽占쎄퐣占쎈쑓占쎌뵠占쎄숲繹먮슣占�
*/ // 獄쎻뫚堉� �눧紐꾩쁽占쎌뿳占쎌몵筌롳옙 占쎈퓠占쎌쑎占쎄텊 野껉퍔�몵嚥∽옙 占쎌굙占쎄맒占쎈쭡.
...
//		System.out.println(i + "-" + (numOfSensingPoints-1) + " [" + sensorPos + " : " + dataBody.indexOf(" ",sensorPos+1) + "] " + data + " " + SensorValue[numOfSensingPoints-1]);
		sensorPos = sensorPos + data.length() + 1;
///////////////////////////////////////////////////////////////////////// Count	- 椰꾨�苑뺜빳占� 占쎌뜎 癰귨옙野껓옙 v1114 	
/*///////////////////////////////////////////////////////////////////////// Count
...
//		System.out.println((numOfSensingPoints-1) + "Count : " + SensorValue[numOfSensingPoints-1]);
	...
	{
		//�뜝�럥�몥�뜝�럩逾졾뜝�럡�댉?�뜝�룞�삕�뜝占�? �뜝�럩逾��뜝�럥�꽑 �뜝�럥�꽑�뜝�럩�읉�뜝�럩逾졾뜝�럥�뱺 �뜝�룞�삕�뜝�럩�궋�뜝�럥由썲뜝�럥堉�.
